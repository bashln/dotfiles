...
- **Grilling**: Conversation with the agent. Uses the /grilling and /architecture skills. Asks one question at a time. The default case.
...
1. Run a `/grilling` and `/architecture` session to surface the open decisions. Ask one question at a time.
...
3. Resolve it, invoking skills as needed. If in doubt, use `/grilling` and `/architecture`.
